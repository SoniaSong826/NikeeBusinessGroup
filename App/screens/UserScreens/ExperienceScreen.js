...
const Item = ({ item }) => (
    <View style={styles.row}>
        <Text style={styles.title}>{item.EmploymentEmployerName} </Text>
        <Text>{item.EmploymentPosition}</Text>
        <View style={styles.item}>
            <Text>{item.EmploymentType}  </Text>
            <Text style={styles.secondaryLabel}>{item.CountryMaster.CountryName}</Text>
        </View>
        <Text style={styles.secondaryLabel}>{convertTime(item.EmploymentCommencedDate) + ' - ' + convertTime(item.EmploymentCompletedDate)}</Text>
    </View>
  );
...
class ExperienceScreen extends React.Component {
    ...
    componentDidMount() {
        // console.log('experience',this.props.details)
    }
        ...
        return (
            <SafeAreaView style={styles.container}>
                <FlatList
                    ...
                    keyExtractor={item => item.EmploymentDetailsID}
                />
            </SafeAreaView>
            // <Text>12313</Text>
        )
